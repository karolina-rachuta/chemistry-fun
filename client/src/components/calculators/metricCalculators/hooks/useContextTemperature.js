...
function useContextTemperature() {
    ...
    function handleUnits(multiplier, substract, unit, value) {
        const result = `${((state.input - substract)* multiplier).toFixed(1)} ${unit}`;
        dispatch({
            type: 'SET_RESULT',
            payload: result,
        });
    ...
    function handleUnitsEnter(defaultValue, substract, defaultUnit, defaultUnit2) {
        const result = `${((state.input - substract)* defaultValue).toFixed(
            1
        )} ${defaultUnit}`;
        dispatch({
            type: 'SET_RESULT',
...
export default useContextTemperature;